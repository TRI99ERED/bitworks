...
use crate::{
    ...
    prelude::{Bitfield128, Bitfield16, Bitfield32, Bitfield8, BitfieldBytes, FlagsEnum, Index},
};
use std::{
...
impl From<BitfieldBytes<8>> for Bitfield64 {
    #[inline(always)]
    fn from(value: BitfieldBytes<8>) -> Self {
...
mod tests {
    ...
    fn combine() -> TestResult {
        let bitfield1 = Bitfield64::new().set_bit(1.try_into()?, true).build();
        let bitfield2 = Bitfield64::new().set_bit(1.try_into()?, true).build();
    ...
    fn split() -> TestResult {
        ...
        assert_eq!(
            bitfield2,
            Bitfield64::new().set_bit(1.try_into()?, true).build()
        );
        assert_eq!(
            bitfield3,
            Bitfield64::new().set_bit(1.try_into()?, true).build()
