...
use crate::{
    ...
    // iter::Bits,
    prelude::{Bitfield128, Bitfield16, Bitfield32, Bitfield64, BitfieldIndex, FlagsEnum},
};
use std::{
...
type Inner = u8;
type BIndex = BitfieldIndex<Bitfield8>;
const BITS: usize = 8;
...
mod tests {
    ...
    fn bits_ref() {
        let bitfield: Tested = 0b11110000.into();
        let mut bit_iter = bitfield.bits_ref();

        assert_eq!(bit_iter.next().as_deref(), Some(&false));
        assert_eq!(bit_iter.next().as_deref(), Some(&false));
        assert_eq!(bit_iter.next().as_deref(), Some(&false));
        assert_eq!(bit_iter.next().as_deref(), Some(&false));
        assert_eq!(bit_iter.next().as_deref(), Some(&true));
        assert_eq!(bit_iter.next().as_deref(), Some(&true));
        assert_eq!(bit_iter.next().as_deref(), Some(&true));
        assert_eq!(bit_iter.next().as_deref(), Some(&true));
        assert_eq!(bit_iter.next(), None);
